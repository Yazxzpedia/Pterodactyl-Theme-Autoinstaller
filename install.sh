...
check_token() {
  ...
  if [ "$USER_TOKEN" = "yazxzdev" ]; then
    echo -e "${GREEN}AKSES BERHASIL${NC}}"
  else
    echo -e "${GREEN}Token Salah! Beli Kode Token Di Yazxzpedia${NC}"
    echo -e "${YELLOW}TELEGRAM : @yazxzpedia${NC}"
    echo -e "${YELLOW}WHATSAPP : +6285624297893${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}© Yazxzpedia${NC}"
    exit 1
  fi
  clear
}
...
install_theme() {
  ...
  THEME_URL="https://github.com/Yazxzpedia/Pterodactyl-Theme-Autoinstaller/raw/main/${THEME_NAME}.zip"
...
# Unduh file tema
wget -O /root/stellar.zip https://github.com/Yazxzpedia/Pterodactyl-Theme-Autoinstaller/raw/main/stellar.zip
...
while true; do
  ...
  echo -e "${RED}'ggg      ,gg'''  .    ggg       Auto Installer Yazxzpedia   ${NC}"
  echo -e "${RED}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${RED}ggg:     gg.     -   ,ggg       • Telegram : Yazxzpedia      ${NC}"
  echo -e "${RED} ggg:     ggg._    _,ggg        • Creadit  : Yazxzpedia  ${NC}"
  echo -e "${RED} ggg.    '.'''ggggggp           • Support by Yazxzpedia  ${NC}"
